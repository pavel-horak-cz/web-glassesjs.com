...
// dist/standalone.js
var DEFAULT_MODEL_CDN = "https://cdn.jsdelivr.net/npm/@mediapipe/tasks-vision/wasm";
var DEFAULT_FRAMES_FOR_RESULT = 30;
var DEFAULT_INTERVAL = 5e3;
var StandaloneDetector = class _StandaloneDetector {
  ...
  static async create(config) {
    const instance = new _StandaloneDetector(config);
  ...
  async initMediaPipe() {
    try {
      ...
      throw new Error(`GlassesJS: Failed to initialize MediaPipe FaceLandmarker. Make sure @mediapipe/tasks-vision is installed or accessible. Error: ${err}`);
    }
  }
  /**
  ...
  async loadVisionModule() {
    ...
    throw new Error("GlassesJS: Could not load @mediapipe/tasks-vision. Install it via npm (npm install @mediapipe/tasks-vision) or ensure CDN access is available.");
  }
  ...
  extractLandmarks() {
    ...
    try {
      ...
      if (result.faceLandmarks && result.faceLandmarks.length > 0 && result.faceLandmarks[0].length >= 478) {
 ...
 * @license MIT
 */
//# sourceMappingURL=glassesjs.standalone.esm.js.map
