 ...
 */
export function sobelEdge(region) {
 ...
 */
export function sobelHorizontal(region) {
    const { data, width, height } = region;
    const result = new Float64Array(width * height);
    for (let y = 1; y < height - 1; y++) {
        for (let x = 1; x < width - 1; x++) {
 ...
 */
export function sobelVertical(region) {
    const { data, width, height } = region;
    const result = new Float64Array(width * height);
    for (let y = 1; y < height - 1; y++) {
        for (let x = 1; x < width - 1; x++) {
 ...
 */
export function regionAboveThreshold(region, threshold) {
