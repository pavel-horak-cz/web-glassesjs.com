...
import { detectIris, extractIrisData } from './methods/iris.js';
...
export class GlassesDetector {
    ...
    getAccumulatedMethods() {
        ...
        return {
            bridge: this.methods.bridge
                ? makeResult(this.frameScores.bridge, (s) => s >= 75 ? 'Strong horizontal edge on nose bridge' :
                ...
                : disabled,
            temple: this.methods.temple
                ? makeResult(this.frameScores.temple, (s) => s >= 75 ? 'Symmetric edges at both temples' :
            ...
            iris: this.methods.iris
                ...
                : disabled,
            depth: this.methods.depth
                ? makeResult(this.frameScores.depth, (s) => s >= 75 ? 'Z-depth discontinuity detected' :
                    s >= 40 ? 'Moderate Z-depth anomaly' :
                        s > 0 ? 'Slight Z-depth variation' :
                            'Normal Z-depth profile (no discontinuity)')
                : disabled,
            contrast: this.methods.contrast
                ? makeResult(this.frameScores.contrast, (s) => s >= 75 ? 'Altered contrast in eye region' :
                ...
                : disabled,
            color: this.methods.color
                ? makeResult(this.frameScores.color, (s) => s >= 75 ? 'Color shift detected in lens area' :
